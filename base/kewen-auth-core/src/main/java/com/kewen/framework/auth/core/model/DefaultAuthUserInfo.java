 ...
 * @since 2023-04-10
 */
public class DefaultAuthUserInfo extends HashMap<String,List<AuthEntity>> implements IUser , AuthUserInfo<AuthEntity> {
     ...
     */
    protected Set<String> propertyNames= Collections.emptySet();
    ...
    private void validProperty(String propertyName){
        boolean contains = this.propertyNames.contains(propertyName);
        if (!contains){
            throw new RuntimeException("this property is not support");
        }
    }
    private void validProperties(Collection<String> propertyNames){
        boolean contains = this.propertyNames.containsAll(propertyNames);
        if (!contains){
            throw new RuntimeException("this property is not support");
        }
    }
    ...
    /*HashMap相关的处理*/
    public DefaultAuthUserInfo(Map<? extends String, ? extends List<AuthEntity>> m) {
        unSupportMethod();
    }
