...
public class AuthorityConvertUtil {
    ...
    /**
     * 解析当前用户的权限
     * @param sysAuthUserInfo
     * @return
     */
    public static Collection<SysAuthority> parseCurrentUser(SysUserInfo sysAuthUserInfo){
        if (sysAuthUserInfo ==null){
            return Collections.singletonList(noneAuthority());
        }
        HashSet<SysAuthority> set = new HashSet<>();
        Optional.ofNullable(sysAuthUserInfo.allDepts()).ifPresent(d-> d.forEach(r -> set.add(to(r))));
        Optional.ofNullable(sysAuthUserInfo.getRoles()).ifPresent(d-> d.forEach(r -> set.add(to(r))));
        return set;
    }
